...
export default function AuthModal({onAuthed}: Props) {
    ...
    const submitCode = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // cas UX clé: link_guest mais email déjà utilisé => proposer merge
            if (apiErr?.status === 409 && inferredPurpose === "link_guest") {
                setSuggestMerge(true);
                setError("Cet email est déjà utilisé. Vous pouvez fusionner vos créations avec ce compte.");
            } else {
                const msg = apiErr ? apiErr.message : err?.message || "Erreur";
                setError(msg);
            }
        } finally {
            setLoading(false);
    ...
    const switchToMerge = async () => {
        // relance un code spécifique merge_into_existing (nouveau code)
        setError(null);
        setLoading(true);
        try {
    ...
    const title = step === "email" ? "Sécurisez vos souvenirs" : "Vérification";
    const desc =
        step === "email"
            ? "Ajoutez un email pour sauvegarder vos créations, exporter et partager sans perdre vos progrès."
            : "Un code vient d’être envoyé. Saisissez-le pour continuer.";
                    ...
                    <div className="mb-4 rounded-2xl border border-stone-200 bg-stone-50 p-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0">
                                <div className="text-sm font-bold text-stone-900">Fusionner avec ce compte</div>
                                <div className="mt-0.5 text-xs leading-relaxed text-stone-600">
                                    On transfère vos créations guest vers le compte existant, puis on vous connecte.
                                </div>
