        ...
        <div className={isLight ? "space-y-6" : "space-y-8"}>
            ...
            >
                ...
                <p className={isLight ? "text-stone-700 text-sm leading-relaxed mt-2" : "text-slate-200 text-sm leading-relaxed mt-2"}>
                    {props.canReplay ? (
                        <>Vous pouvez revoir l’aperçu ici. Débloquez l’accès pour télécharger l’original et garder le lien.</>
                    ) : (
                        <>La vidéo se prépare… Elle apparaîtra ici automatiquement dès qu’elle est prête.</>
                    )}
                </p>

                <div className={isLight ? "mt-3 flex items-center gap-2 text-[11px] text-stone-500" : "mt-3 flex items-center gap-2 text-[11px] text-slate-400"}>
                    <Heart size={14} className={isLight ? "text-rose-400" : "text-cyan-300"} />
                    <span className="font-mono">{props.statusLine}</span>
                    <span className={isLight ? "ml-auto font-mono text-stone-500 tabular-nums" : "ml-auto font-mono text-slate-500 tabular-nums"}>
            {pct}%
          </span>
                </div>
            ...
            <div className="space-y-3">
                <h1 className={isLight ? "text-3xl font-serif text-stone-900 leading-tight" : "text-4xl font-bold leading-tight tracking-tight"}>
                    Récupérez ce{" "}
                    <span className={isLight ? "italic text-stone-800" : "text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400"}>
            moment unique
          </span>
                    .
                </h1>
                ...
                {expiresAtMs != null ? (
                    ...
                    >
                        <div className={isLight ? "p-2.5 bg-white rounded-full border border-rose-200" : "p-2.5 bg-red-500/20 rounded-full z-10"}>
                        ...
                        <div>
                            ...
                            <div className={isLight ? "text-3xl font-mono font-bold text-stone-900 tabular-nums" : "text-3xl font-mono font-bold text-white tabular-nums leading-none tracking-tight"}>
                                {formatTime(timeLeftSec)}
                            </div>

                            <div className={isLight ? "text-[11px] text-stone-500 font-mono mt-1" : "text-[11px] text-slate-400 font-mono mt-1"}>
                ...
                <p className={isLight ? "text-stone-600 text-sm leading-relaxed" : "text-slate-400 text-sm leading-relaxed border-l-2 border-indigo-500/30 pl-4"}>
                    Débloquez l’accès pour télécharger la version originale et conserver le lien dans votre espace Lifee.
                </p>
            </div>
            ...
            <div className="space-y-3 pt-1">
                ...
                >
                    <Download size={18} className={isLight ? "text-white" : "group-hover:animate-bounce text-indigo-600"} />
